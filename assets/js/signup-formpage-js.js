...
function zc_selectUserType(val)    {
    ...
    if(jQuery("#unsaved_list").html() == "") {
        ...
        // alert("No Mailing list available from Zoho-Campaigns to show. Either create new Mailing List or check for the previlages to access it.");
    }
}
...
function zc_uiSliderChangeSetter(idval) {
    $zc_j1('#slider_' + idval).slider({
        ...
        $zc_j1("#sliderToolTip_" + idval).css("margin-left", $zc_j1("#slider_" + idval).find("a").position().left - 17);
    ...
    $zc_j1('#slider_' + idval).bind('mouseup', function (event) {
        var left = $zc_j1("#slider_" + idval).find("a").position().left;
        zc_slider_on = false;
...
function zc_formRetreiver(id, listkey) {
    ...
    var form_url = zc_domain_url + "/api/xml/formdetails?authtoken=" + zc_on_load_apikey_val + "&scope=CampaignsAPI&version=1&resfmt=json&listkey=" + listkey /*+ "&resform=" + formname*/+"&userDomain=wordpress&signupformid="+signupformid;
...
function zc_addForm(idval,signupformid)  {
    ...
    var form_url = zc_domain_url + "/api/xml/formdetails?authtoken=" + zc_on_load_apikey_val + "&scope=CampaignsAPI&version=1&resfmt=json&listkey=" + listkey + "&userDomain=wordpress&signupformid=" + signupformid;
    jQuery.ajax({
        type: 'POST',
        url: form_url,
        success: function (responseData, textStatus, jqXHR) {
...
function zc_createNewForm() {
    ...
    if(zc_first_load || zc_saved_idval.length == zc_all_idval.length) {
        ...
        jQuery.ajax({
            ...
            error: function (responseData, textStatus, errorThrown) {
                console.log("error in getting list details")
                if(jQuery("#unsaved_list").html() == "" || jQuery("#unsaved_list").html().indexOf("No mailing list") >=0 ) {
                    jQuery("#unsaved_list").html('<div class=" zcmt15 zctxt zctcenter zcf14" id="no_list_message"><h4>No mailing list found. Please add a mailing list in your Zoho Campaigns account before proceeding with sign-up form.</h4></div>');
                    // alert("No Mailing list available from Zoho-Campaigns to show. Either create new Mailing List or check for the previlages to access it.");
                }
...
function zc_editorContentChecker(me,option,authToken, listkey) {
    ...
    if (option == '0') {
        ...
        var form_url = zc_domain_url + "/api/xml/formdetails?authtoken=" + authToken + "&scope=CampaignsAPI&version=1&resfmt=xml&listkey=" + listkey + "&resform=" + formname;
        jQuery.ajax({
            type: 'POST',
            url: form_url,
            success: function (responseData, textStatus, jqXHR) {
            ...
            error: function (responseData, textStatus, errorThrown) {
                alert("This list has been removed from your Zoho-Campaigns account or there has been a change in the previlage.");
                if(delete_form_type == "zoho") {
    ...
    jQuery.ajax({
        ...
        success: function (responseData, textStatus, jqXHR) {
            if (option == '1') {
                ...
                if(zc_ver == '1')   {
                    ...
                    if(formType == '0') {
                        ...
                        var link = zc_domain_url;
                        var trackingText = "wordpress";
                        var captchaPath = link + "/CaptchaVerify.zc?mode=generate";
                        ...
                        jsCode +="});\n\n";//No I18N
                        jsCode +="/*Load Captcha For this*/ \n loadCaptcha('"+captchaPath+"');\n\n /*Tracking Enabled*/ \n trackSignupEvent(trackingText);\n ";//No I18N
                        jsCode +="});\n";//No I18N
                        jsCode +="<\/script>\n";//No I18N
            ...
            // console.log(document.getElementById("hidden_zcform_textarea_"+idval).value)
            // console.log(document.getElementById("hidden_simple_textarea_"+idval).value)
            // console.log(jQuery("#form_"+idval))
